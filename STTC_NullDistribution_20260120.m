...
for nUniqueWells = 1:length(UniqueWells)
    ...
    NumElectrodes = length(ElectrodesInCurrentWell);

% Loop over ElectrodesInCurrentWell
    for ii = 1:NumElectrodes
        Electrode1 = ElectrodesInCurrentWell(ii);
        for iii = 1:NumElectrodes
            Electrode2 = ElectrodesInCurrentWell(iii);

% Calculate STTC between Electrode1 and ElectrodesInCurrentWell
...
if ~isempty(FilteredTable)
    MaybeCall(SelectedAnalysis, 'STTCVisualization', FilteredTable, ChannelLayout, ElectrodeNumber, base) 
end 
