...
OptionalAnalysis = AnalysisCatalog_20260812();
SelectedAnalysis = SelectOptionalScripts(OptionalAnalysis);

nFiles = numel(DataFileList);
for z = 1:nFiles
    ...
    clear  LowPassOnData HighPassOnData  outputPath;   %removes data pulled in from file to make room

    %creating Array to keep Parameters Below!!!
    Settings=[];

    Settings.Recording.SamplingRate = SamplingRate;
    ...
    Settings.BurstDetection.MinSpikes = 3;                      % min of 3 spikes to be a burst (MaxInterval)
    ...
    clear aHigh bHigh aLow bLow Filters i nChannels nFilters
    disp('Data Filtered')
    ...
    FiringFreqAll = cellfun(@(x) numel(x)/DurationS, AllPeaks(:,1));           % finding firing rate based on number of entries for peaks
    ...
    WindowStart = {};
    WindowEnd = {};
    BeforeEventMs = Settings.Waveform.BeforeEventMs;
    AfterEventMs = Settings.Waveform.AfterEventMs;
    for i = 1:nElectrodes
        WindowStart{i} = SpikeIdx{i, 1} - (SamplesPerMs * BeforeEventMs);   % 1 ms before
        WindowEnd{i} = SpikeIdx{i, 1} + (SamplesPerMs * AfterEventMs);     % 2.2 after

        nWindows = numel(WindowStart{i});
        for ii = 1:nWindows
    ...
    for i = 1:nElectrodes
        nWindows = numel(WindowStart{i});
        ElectrodeWaveforms = cell(nWindows, 1);
    ...
    clear nWindows Idxpeak i ii j SpikeIdx AfterEventMs BeforeEventMs nElectrodes 
    ...
    nSpikesByElectrode = numel(SpikeByElectrode);

    for i = 1:nSpikesByElectrode
        ...
        SpikeByElectrode{i}.TotalSpikes = numel(SpikeByElectrode{i}.TimingS);
        ...
        nISI = numel(SpikeByElectrode{i}.SpikeISI);
        SpikeByElectrode{i}.SpikeISIIdx = (1:nISI)';
    end

    % for Burst
    BurstByElectrode = cell(size(AllPeaks(:, 1)));
    % using nSpikesByElectrode since BurstByElectrode and SpikeByElectrode
    % are the same size
    for i = 1:nSpikesByElectrode
        ...
        nISI = numel(BurstByElectrode{i}.BurstISI);
        BurstByElectrode{i}.BurstISIIdx = (1:nISI)';
    ...
    %nSpikesByElectrode for same reason as above
    for i = 1:nSpikesByElectrode
    ...
    nUniqueWells = numel(UniqueWells);

    for i = 1:nUniqueWells
    ...
    MaxInterBurstInterval = Settings.BurstDetection.MaxInterBurstInterval;
    MinDuration = Settings.BurstDetection.MinDuration;
    ...
    for i = 1:nSpikesByElectrode
        ...
        nBurstStartIdxs = numel(BurstStartISIIdx);
        for ii = 1:nBurstStartIdxs
            Burst1 = [];
            nISI = numel(BurstByElectrode{i}.BurstISI);
            for iii = BurstStartISIIdx(ii):nISI
        ...
        nProcessingBurstIdxs = numel(ProcessingBurstIdxs);
        for viii = 1:nProcessingBurstIdxs
            if numel(ProcessingBurstIdxs{1, viii}) + 1 < MinBurstSpikes
        ...
        nProcessingBurstIdxs = numel(ProcessingBurstIdxs);
        for v = 1:nProcessingBurstIdxs-1

            if isempty(ProcessingBurstIdxs{v})           %In case no bursts, moves along
                continue
            end

            for vi = v+1:nProcessingBurstIdxs
                % merging bursts within the IBI threshold
                if isempty(ProcessingBurstIdxs{vi})
                    continue
                end
                if SpikeByElectrode{i}.TimingS(ProcessingBurstIdxs{vi}(1)) - SpikeByElectrode{i}.TimingS(ProcessingBurstIdxs{v}(end)+1) < MaxInterBurstInterval
        ...
        nProcessingBurstIdxs = numel(ProcessingBurstIdxs);
        for vii = 1:nProcessingBurstIdxs
            if SpikeByElectrode{i}.TimingS(ProcessingBurstIdxs{1, vii}(end)) - SpikeByElectrode{i}.TimingS(ProcessingBurstIdxs{1, vii}(1)) < MinDuration
    ...
    clear nSpikesByElectrode MinDuration MaxInterBurstInterval nISI nBurstStartIdxs nMergeDupe nProcessingBurstIdxs
    disp('Burst Detection Complete')

    %% Getting timings back for bursts, not using idxs

    % Swap ISI index to spike indices
    nBurstbyElectrodes = numel(BurstByElectrode);
    for i = 1:nBurstbyElectrodes
        ...
        nBurstISIIdx = numel(BurstByElectrode{i}.BurstISIIdx);
        for ii = 1:nBurstISIIdx
        ...
        nBurstIdxs = numel(BurstByElectrode{i}.BurstIdxs);
        for iii = 1:nBurstIdxs
    ...
    clear Burst1 Burst i ii iii iv v nBurstIdxs nBurstISIIdx
    disp('Bursts Sorted')
    ...
    for i = 1:nBurstbyElectrodes
        ...
        BurstByElectrode{i}.NumberOfBursts = numel(BurstByElectrode{i}.BurstISIIdx);
            ...
            IBI2 = [];
            nBursts = numel(BurstByElectrode{i}.TimingS);
    ...
    clear AvgISIInBursts SDISIBurst i ii iii iv IBI2 InterBurstInterval DurationM nBurstbyElectrodes
    ...
    for i = 1:nUniqueWells
        ...
        nBurstByElectrode = numel(BurstByElectrode);
        for ii = 1:nBurstByElectrode
        ...
        if numel(BurstingElectrodesInWell) < Settings.NetworkDetection.MinChannelParticipating
        ...
        % electrodes
        TimeSortedBursts = [];
        nBurstingElectrodesInWell = numel(BurstingElectrodesInWell);
        for iii = 1:nBurstingElectrodesInWell
            nBurstByElectrodeTimes = numel(BurstByElectrode{BurstingElectrodesInWell(iii)}.TimingS);
            for v = 1:nBurstByElectrodeTimes
        ...
        UniqueElecByNetwork = [];
        nNetworkElectrodesInvolved = numel(NetworkBurstInfo{i}.ElectrodesInvolved);
        for l = 1:nNetworkElectrodesInvolved
        ...
        nNetworkBurstingElectrodes = numel(NetworkBurstInfo{i}.NetworkBurstingElectrodes);
        for m = 1:nNetworkBurstingElectrodes
    ...
    clear nUniqueWells nNetworkBurstingElectrodes nNetworkElectrodesInvolved nBurstByElectrodeTimes nBurstingElectrodesInWell BurstIdxs
    ...
    nIndependentWells = numel(NetworkBurstInfo);
    for i = 1:nIndependentWells  %independent wells
        if NetworkBurstInfo{i}.NumberOfNetworkBursts ~= 0
            NetworkDurationS = [];
            TempCount = zeros(size(SpikeByElectrode, 1), 1);

            % End - Start Times for Duration S
            nBursts = size(NetworkBurstInfo{i}.NetworkBursts,1);
            for ii = 1:nBursts %Each Burst
                ...
                for iii = 1:numel(NetworkBurstInfo{i}.NetworkBursts{ii, 3}) %Each electrode in burst
            ...
            for iv = 1:size(NetworkBurstInfo{i}.NetworkBursts, 1) %Each Network Burst
                ...
                for v = 1:length(UniqueElecByNetwork) %Each unique Electrode in Burst to prevent duplicate times
                    ...
                    temp = [];
                    temp2 = [];
                    for vi = 1:size(NetworkByElectrode{ElectrodeRow}.TimingS,1)
    ...
    clear nBursts nIndependentWells 
    disp('Network Sorted by Electrode')

    %% Network Characteristics
    nNetworkByElectrode = numel(NetworkByElectrode);
    for i = 1:nNetworkByElectrode
        ...
        if NetworkByElectrode{i}.NetworkBurstCount ~= 0
            ...
            NetworkByElectrode{i}.PercentSpikesInNetworkBurst = ((NetworkByElectrode{i}.AvgNetworkSpikeCount*NetworkByElectrode{i}.NetworkBurstCount)/numel(AllPeaks{i}(:)))*100;
    ...
    clear Tempcount i ii iii v ElectrodeRow TempTime InterNetworkInterval nNetworkByElectrode
    disp('Network Burst Characteristics Done')

    %% empty Cell to NaN
    nSpikeByElectrode = numel(SpikeByElectrode);
    for i = 1:nSpikeByElectrode
    ...
    clear i nBurstByElectrode nNetworkByElectrode nSpikeByElectrode SpikeByElectrode NetworkByElectrode BurstByElectrode
