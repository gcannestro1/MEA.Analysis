...
disp('Running Clustering Coefficient')
...
for i = 1:height(FilteredTable)
    if string(FilteredTable.Electrode1(i)) == string(FilteredTable.Electrode2(i))
        ToRemove = [ToRemove, i];
    end
end
...
for i = 1:length(UniqueClusterWells)
    ...
    %% Plotting here (as is for electrode labels)
    disp('Plotting')

    %    Replaced node labels for 1:12 for consistency
    % labels = string(1:12);


    % F = figure;
    % h = plot(G, 'XData', XCoords, 'YData', YCoords, 'NodeLabel', labels, 'NodeFontSize', 14, 'LineWidth', 1, 'EdgeColor', 'k', 'NodeColor', '#808080');
    % highlight(h, ActiveElecForWell, 'NodeColor', 'r')
    % title(['Clustering Coefficient C = ' num2str(AvgClusteringCo)])
    % set(gca, 'XTick', [], 'YTick', [])
    % axis off
    %
    % %saving figure as .png with proper resolution
    ...
    % Electrode setup with weighted images
    % F = figure;
    % h = plot(G, 'XData', XCoords, 'YData', YCoords, 'NodeLabel', labels, 'NodeFontSize', 14, 'LineWidth', 1, 'NodeColor', '#808080');
    % highlight(h, ActiveElecForWell, 'NodeColor', 'r')
    % cmap = colormap('jet');
    % zmap = linspace(0, 1, length(cmap));
    % color = interp1(zmap, cmap, G.Edges.Weight);
    % h.EdgeColor = color;
    % colorbar
    % title(['Clustering Coefficient C = ' num2str(AvgClusteringCo)])
    % set(gca, 'XTick', [], 'YTick', [])
    ...
    % Plot without Electrode setup

    %option 1: no elec setup, coloration same as previous
    % F = figure;
    % h = plot(G, 'NodeColor', 'k', 'EdgeColor', 'k');
    % highlight(h, ActiveElecForWell, 'NodeColor', 'r')
    % title(['Clustering Coefficient C = ' num2str(AvgClusteringCo)])
    % set(gca, 'XTick', [], 'YTick', [])
    % axis off
    ...
    % option 2: black electrodes, weighted lines
    % F = figure;
    % h = plot(G, 'NodeColor', 'k', 'LineWidth', 2);
    %
    ...
    % option 3: reg electrode, weighted lines
    F = figure;
    h = plot(G, 'NodeColor', 'k', 'LineWidth', 2);
    highlight(h, ActiveElecForWell, 'NodeColor', 'r')
    ...
    cmap = colormap('jet');
    zmap = linspace(0, 1, length(cmap));
    color = interp1(zmap, cmap, abs(G.Edges.Weight));
    colorbar
    h.EdgeColor = color;
    title(['Clustering Coefficient C = ' num2str(AvgClusteringCo)])
    set(gca, 'XTick', [], 'YTick', [])
    ...
    FigFile = fullfile(sprintf('%s%sClusterCoNoElec%s', base, string(UniqueClusterWells(i))));
    FigFile = append(FigFile, '.png');
    exportgraphics(gcf, FigFile, 'Resolution', 600)


    % %Adjacency matrix
    % h = adjacency(G, G.Edges.Weight);
    % cmap = colormap('jet');
    % zmap = linspace(0, 1, length(cmap));
    % color = interp1(zmap, cmap, G.Edges.Weight);
    % clims = [0 1];
    % imagesc(h,clims)
    % colorbar
    % title(['Adjacency Matrix Clustering Coefficient C = ' num2str(AvgClusteringCo)])
